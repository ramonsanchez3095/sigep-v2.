...
import type { DRComparisonRow } from '@/lib/delitos-rurales-definition';
...
const DR_GRADIENT = (color: string) =>
...
function TableHeader({
  ...
  return (
    <div
      ...
      style={{ background: DR_GRADIENT(color) }}
    >
...
// ─────────────────────────────────────────────────────────────────────────────
// DRComparisonTable – Tabla comparativa para Delitos Rurales
// ─────────────────────────────────────────────────────────────────────────────
interface DRComparisonTableProps {
  title: string;
  badge?: string;
  color: string;
  rows: DRComparisonRow[];
  labelAnterior: string;
  labelActual: string;
  canSave?: boolean;
  onSave?: (rows: DRComparisonRow[]) => Promise<void>;
  charColumnName?: string;
}

export function DRComparisonTable({
  title, badge, color, rows, labelAnterior, labelActual, canSave, onSave, charColumnName = 'Detalle'
}: DRComparisonTableProps) {
  const { edicionHabilitada } = useAppStore();
  const [editando, setEditando] = useState(false);
  const [draft, setDraft] = useState<DRComparisonRow[]>(rows);
