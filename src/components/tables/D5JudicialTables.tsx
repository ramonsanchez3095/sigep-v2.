...
import type { D5ComparisonRow, D5Mes } from '@/lib/d5-definition';
import { D5_MESES } from '@/lib/d5-definition';
...
const VIOLET_GRADIENT = (color: string) =>
...
function TableHeader({
  ...
  return (
    <div
      ...
      style={{ background: VIOLET_GRADIENT(color) }}
    >
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE 1: D5ComparisonTable (Tablas 1, 3, 6, 7)
// ─────────────────────────────────────────────────────────────────────────────
interface D5ComparisonTableProps {
  title: string;
  badge?: string;
  color: string;
  rows: D5ComparisonRow[];
  labelAnterior: string;
  labelActual: string;
  canSave?: boolean;
  onSave?: (rows: D5ComparisonRow[]) => Promise<void>;
  charColumnName?: string;
}

export function D5ComparisonTable({
  title, badge, color, rows, labelAnterior, labelActual, canSave, onSave, charColumnName = 'Características'
}: D5ComparisonTableProps) {
  const { edicionHabilitada } = useAppStore();
  const [editando, setEditando] = useState(false);
  const [draft, setDraft] = useState<D5ComparisonRow[]>(rows);
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE 2: D5HabeasCorpusTable (Tabla 5: meses como filas con selector)
// ─────────────────────────────────────────────────────────────────────────────
export function D5HabeasCorpusTable({
  ...
  const [selectedMonths, setSelectedMonths] = useState<Set<D5Mes>>(
    new Set(D5_MESES.slice(0, 7)) // Predeterminado: Enero a Julio
  );
  const [showMonthPicker, setShowMonthPicker] = useState(false);

  const active = editando ? draft : rows;

  // Filtrar filas según meses seleccionados
  const visibleRows = active.filter(r => selectedMonths.has(r.id.toUpperCase() as D5Mes));
  ...
  // Totales dinámicos
  const totalAnterior = visibleRows.reduce((acc, r) => acc + r.periodoAnterior, 0);
  const totalActual = visibleRows.reduce((acc, r) => acc + r.periodoActual, 0);
  const difTotal = totalActual - totalAnterior;
  const porcTotal = totalAnterior === 0 ? (totalActual > 0 ? 100 : 0) : Math.round(((totalActual - totalAnterior) / totalAnterior) * 100);

  const MONTH_NAMES_ES: Record<D5Mes, string> = {
    ...
    <div className="card overflow-hidden rounded-[26px] border border-slate-200/80 shadow-[0_16px_38px_rgba(15,29,48,0.08)] bg-white">
      ...
      >
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur-sm">
            <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/70">
              Total visible visible ({labelAnterior.match(/\d{4}/)?.[0] ?? 'Anterior'})
            </div>
            <div className="mt-1 text-xl font-black text-white tabular-nums">
              {formatNumber(totalAnterior)}
            </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur-sm">
            <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/70">
              Total visible visible ({labelActual.match(/\d{4}/)?.[0] ?? 'Actual'})
            </div>
            <div className="mt-1 text-xl font-black text-white tabular-nums">
              {formatNumber(totalActual)}
            </div>
          ...
          <tbody>
            {visibleRows.map((row) => {
              ...
              return (
                <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                  <td className="px-4 py-3 font-semibold uppercase border-r border-slate-200/60 text-slate-900 bg-slate-50/30">
                    {MONTH_NAMES_ES[row.id.toUpperCase() as D5Mes]}
                  </td>
                  <td className="px-4 py-3 text-center font-semibold tabular-nums border-r border-slate-200/60">
                    {editando ? (
                      <input
            ...
            {/* Fila de Totales */}
            <tr className="bg-slate-900 text-white font-bold hover:bg-slate-900 border-t border-slate-700">
              <td className="px-4 py-3 uppercase bg-slate-950 border-r border-slate-700">TOTAL VISIBLE</td>
              <td className="px-4 py-3 text-center border-r border-slate-700 tabular-nums">
                {formatNumber(totalAnterior)}
              </td>
              <td className="px-4 py-3 text-center border-r border-slate-700 tabular-nums">
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE 3: D5MonthlyGroupedTable (Tablas 2 y 4: meses como columnas con selector)
// ─────────────────────────────────────────────────────────────────────────────
...
interface D5MonthlyGroupedTableProps {
  title: string;
  badge?: string;
  color: string;
  concepts: Array<{ id: string; label: string }>;
  rows: D5MonthlyInputRow[];
  canSave?: boolean;
  onSave?: (updatedRows: D5MonthlyInputRow[]) => Promise<void>;
  charColumnName?: string;
}

export function D5MonthlyGroupedTable({
  title, badge, color, concepts, rows, canSave, onSave, charColumnName = 'Detenidos'
}: D5MonthlyGroupedTableProps) {
  const { edicionHabilitada } = useAppStore();
  const [editando, setEditando] = useState(false);
  const [draft, setDraft] = useState<D5MonthlyInputRow[]>(rows);
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();
  ...
  const active = editando ? draft : rows;
  const visibleMonths = D5_MESES.filter(m => selectedMonths.has(m));

  const handleEdit = () => { setDraft(rows); setEditando(true); };
  const handleCancel = () => { setEditando(false); setError(null); };
  const handleChange = (conceptId: string, mes: D5Mes, anio: 2024 | 2025, val: string) => {
    const n = Number(val);
    setDraft(d => d.map(r =>
  ...
  const renderCell = (conceptId: string, mes: D5Mes, anio: 2024 | 2025) => {
    ...
    return editando ? (
      <input
        ...
        className="w-16 rounded-lg border border-slate-300 px-1.5 py-1 text-center text-slate-900 focus:border-[#8b5cf6] outline-none text-xs"
      />
    ) : (
      formatNumber(val)
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE 4: D5SingleYearMonthlyTable (Tabla 10: un solo año 2025 con selector)
// ─────────────────────────────────────────────────────────────────────────────
export function D5SingleYearMonthlyTable({
  title, badge, color, concepts, rows, canSave, onSave, charColumnName = 'Concepto'
}: D5MonthlyGroupedTableProps) {
  const { edicionHabilitada } = useAppStore();
  const [editando, setEditando] = useState(false);
  const [draft, setDraft] = useState<D5MonthlyInputRow[]>(rows);
  ...
  const [selectedMonths, setSelectedMonths] = useState<Set<D5Mes>>(
    new Set(D5_MESES.slice(2, 7)) // Predeterminado: Marzo a Julio para las llamadas de antecedentes
  );
  const [showMonthPicker, setShowMonthPicker] = useState(false);
  ...
  const handleEdit = () => { setDraft(rows); setEditando(true); };
  const handleCancel = () => { setEditando(false); setError(null); };
  const handleChange = (conceptId: string, mes: D5Mes, val: string) => {
    const n = Number(val);
    setDraft(d => d.map(r =>
      r.concept === conceptId && r.mes === mes && r.anio === 2025
        ? { ...r, valor: isFinite(n) ? n : 0 }
        : r
    ));
  };
  ...
  // Totales por concepto (año 2025 únicamente)
  const getSumForConcept = (conceptId: string) => {
    return active
      .filter(r => r.concept === conceptId && r.anio === 2025 && selectedMonths.has(r.mes))
      .reduce((sum, r) => sum + r.valor, 0);
  };

  const totalGrand2025 = getSumForConcept('con_novedad') + getSumForConcept('sin_novedad');

  const renderCell = (conceptId: string, mes: D5Mes) => {
    const found = active.find(r => r.concept === conceptId && r.mes === mes && r.anio === 2025);
    const val = found?.valor ?? 0;
    ...
    return editando ? (
      <input
        type="number"
        value={val}
        onChange={e => handleChange(conceptId, mes, e.target.value)}
        className="w-16 rounded-lg border border-slate-300 px-1.5 py-1 text-center text-slate-900 focus:border-[#8b5cf6] outline-none text-xs"
      />
    ) : (
      formatNumber(val)
    ...
    <div className="card overflow-hidden rounded-[26px] border border-slate-200/80 shadow-[0_16px_38px_rgba(15,29,48,0.08)] bg-white">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse text-sm text-slate-700">
          <thead>
            <tr className="bg-slate-100/80 text-slate-700">
              <th className="border-b border-r border-slate-200 px-4 py-3 text-left font-semibold uppercase tracking-[0.08em] min-w-[200px]">
                {charColumnName}
              </th>
              {visibleMonths.map(mes => (
                <th key={mes} className="border-b border-r border-slate-200 px-2 py-3 text-center font-semibold uppercase text-xs tracking-wider">
                  {MONTH_NAMES_ES[mes].toUpperCase()}
                </th>
              ))}
              <th className="border-b border-slate-200 px-3 py-3 text-center font-semibold uppercase tracking-[0.08em] w-28">
                Total
              </th>
            </tr>
          </thead>
          <tbody>
            {concepts.map((concept) => {
              const sum2025 = getSumForConcept(concept.id);

              return (
                <tr key={concept.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                  <td className="px-4 py-3 font-semibold uppercase border-r border-slate-200/60 text-slate-900 bg-slate-50/30">
                    {concept.label}
                  </td>
                  {visibleMonths.map(mes => (
                    <td key={mes} className="px-3 py-2.5 text-center border-r border-slate-200/60 tabular-nums">
                      {renderCell(concept.id, mes)}
                    </td>
                  ))}
            ...
            {/* Fila de Totales (LLAMADAS) */}
            <tr className="bg-slate-900 text-white font-bold hover:bg-slate-900 border-t border-slate-700">
              <td className="px-4 py-3 uppercase bg-slate-950 border-r border-slate-700">
                LLAMADAS
              </td>
              {visibleMonths.map(mes => {
                const conNovedadVal = active.find(r => r.concept === 'con_novedad' && r.mes === mes && r.anio === 2025)?.valor ?? 0;
                const sinNovedadVal = active.find(r => r.concept === 'sin_novedad' && r.mes === mes && r.anio === 2025)?.valor ?? 0;
                const mSum = conNovedadVal + sinNovedadVal;
                return (
                  <td key={mes} className="px-3 py-2.5 text-center border-r border-slate-700/60 tabular-nums bg-slate-950/40">
                    {formatNumber(mSum)}
                  </td>
                );
              })}
              <td className="px-3 py-2.5 text-center tabular-nums bg-slate-950 text-white font-black">
                {formatNumber(totalGrand2025)}
              </td>
            </tr>
          </tbody>
